// components/badges/BadgeSection.js
        ...
        <div className="flex flex-col gap-8">
            ...
            <div className="p-6 bg-card-light dark:bg-card-dark rounded-xl border border-border-light dark:border-border-dark">
                ...
                <div className="w-full bg-background-light dark:bg-background-dark rounded-full h-3 mb-2">
                    <div className="bg-primary h-3 rounded-full" style={{ width: `${progress.percentage}%` }}></div>
                </div>
                <div className="flex justify-between text-xs text-subtext-light dark:text-subtext-dark">
                    <span>{progress.current}/{progress.target} Countries visited</span>
            ...
            {/* Heading text is now smaller, consistent with BadgeSection changes */}
            <h3 className="text-text-light dark:text-text-dark text-base sm:text-lg font-bold leading-tight tracking-[-0.015em] mb-4 sm:mb-6 flex items-center gap-2">
                <span className="material-symbols-outlined text-subtext-light">military_tech</span>
                Future Achievements
            </h3>
            {/* Gap reduced from gap-6 to gap-3 for a denser list */}
                ...
                {badges.future.map((badge, index) => (
                    ...
                    >
                        <div className="size-10 sm:size-12 lg:size-14 flex items-center justify-center rounded-full bg-gray-200 dark:bg-gray-700 text-gray-500 flex-shrink-0">
                            <span className="material-symbols-outlined text-lg sm:text-xl lg:text-2xl">{badge.icon}</span>
                        </div>
                        
                        {/* Text Block */}
                        <div className="flex flex-col">
                            {/* Title: Bumped up to text-sm for hierarchy */}
                            <p className="font-bold text-text-light dark:text-text-dark text-sm truncate">{badge.title}</p>
                            
                            {/* Description: Smallest size (text-xs) */}
                            <p className="text-subtext-light dark:text-subtext-dark text-xs overflow-hidden leading-snug">{badge.desc}</p>
                        </div>
                    </div>
                ))}
            </div>
