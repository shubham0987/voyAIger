...
const StatItem = ({ count, label }) => (
  <div className="flex flex-col gap-1 rounded-lg border border-border-light dark:border-border-dark p-3 bg-background-light dark:bg-background-dark">
    <p className="text-text-light dark:text-text-dark tracking-light text-base sm:text-lg font-bold leading-tight">
      {count}
    </p>
    <p className="text-subtext-light dark:text-subtext-dark text-xs font-normal leading-normal">
      {label}
    </p>
  </div>
);
      ...
      <div className="flex flex-col gap-6 p-6 bg-card-light dark:bg-card-dark rounded-xl border border-border-light dark:border-border-dark">
        <div className="flex flex-col items-center gap-4 text-center">
          ...
          <div className="flex flex-col justify-center">
            <p className="text-text-light dark:text-text-dark text-lg sm:text-xl font-bold leading-tight tracking-[-0.015em]">
              {profile.name}
            </p>
            <p className="text-subtext-light dark:text-subtext-dark text-xs font-normal leading-normal">
              @{profile.handle}
        ...
        <h3 className="text-text-light dark:text-text-dark text-lg font-bold leading-tight tracking-[-0.015em] pt-2">
          Interests
        </h3>

        <div className="flex gap-2 flex-wrap">
          {/* Interests List or Placeholder */}
          {profile.interests && profile.interests.length > 0 ? (
            // Renders the list of interests if the array is NOT empty
            <div className="flex gap-2 flex-wrap">
              {profile.interests.map((interest, index) => (
                ...
                >
                  <p
                    className="text-subtext-light dark:text-subtext-dark text-xs font-medium leading-normal 
                              transition-colors"
                  >
                    {interest}
                  </p>
            ...
            // Renders the call-to-action placeholder if the array IS empty
            <div className="flex items-center justify-center p-3 text-center rounded-lg border border-dashed border-border-light dark:border-border-dark cursor-pointer transition-colors duration-200 hover:bg-background-light dark:hover:bg-background-dark/50">
              <span className="material-symbols-outlined text-base text-subtext-light dark:text-subtext-dark mr-2">
                add
              </span>
              <p className="text-subtext-light dark:text-subtext-dark text-xs font-medium">
                Click here to add your interests
              </p>
            </div>
          )}
        </div>
